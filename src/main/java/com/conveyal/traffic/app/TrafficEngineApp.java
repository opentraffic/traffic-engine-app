...
public class TrafficEngineApp {
	...
	get("/weeks",
		(request, response) -> {
		    ...
		    List<Integer> weeks = engine.getTrafficEngine().osmData.statsDataStore
			    .getWeekList();
	...
	get("/weeklyStats",
		(request, response) -> {
		    ...
		    Set<Long> segmentIds = TrafficEngineApp.engine
	...
	get("/route",
		(request, response) -> {
			...
			List<SpatialDataItem> streetSegments = engine
				.getTrafficEngine()
